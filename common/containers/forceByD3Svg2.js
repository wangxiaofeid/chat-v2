...
class forceByD3Svg2 extends Component{
    ...
    drew(){
        ...
        self.nodes.forEach(function(d){
            ...
            if(dom){
                dom.setAttribute('transform', 'translate('+(d.x||defaultWidth)+','+(d.y||defaultHeight)+')');
            }
        });
    }
                    ...
                    <g className="outg">
                      <g className="lines">
                        ...
                        this.links.map(function(link,index){
                            return (<line ref={ 'n'+link.source.name+link.target.name } key={index} className="line" style={{strokeWidth: 1}}></line>)
                            }.bind(this))
                        }
                      </g>
                      <g className="nodes">
                        ...
                        this.nodes.map(function(node,index){
                            ...
                            return (<g className="node" ref={ 'n'+node.name } key={index}><circle r={radius} style={{fill: this.state.color(node.category) }}></circle></g>)
                            }.bind(this))
                        }
                      </g>
                    </g>
